...
ui <- fluidPage(
  ...
  fluidRow(
    ...
    column(width = 6,
           ...
           div(
             ...
             textInput("genename", "Enter marker name"),
             actionButton("ftrplot", "See expression", style = "margin-top:20px ; margin-bottom:20px"),
             plotOutput("plotfeatureplot", height = "auto")
           ))
  ),
  ...
  ### 3- Inspect genes expression     ---------------------------------------------
  p("3- Expression inspection",
    style = "color:darkred ; font-weight:600 ; font-size:160% ; background-color:gold ; 
    margin-bottom:20px ; margin-top:30px"),
      ...
      fluidRow(
        column(width = 3,
               ...
               sliderInput("go.maxgeneset", "Limit the geneset size", value = 500,
                           min = 200, max = 5000, step = 100, width = "300px"),
  ...
  fluidRow(
    column(width = 3,
           selectInput("gsea.sel", "Select DEGs", choices = c()),
           div(
             style = "margin-bottom:50px ; margin-top:20px",
             sliderInput("gsea_GsSize", "GeneSet size range", min = 10, max = 2000, step = 10, value = c(10, 500))
           ),
           numericInput("nPermSimple", "nPermSimple", value = 1000),
           actionButton("run.gsea", "Run GSEA", style = "margin-top:20px")),
    column(width = 9,
           div(
           ...
           div(
             ...
             column(width = 5,
                    ...
                    actionButton("gseaplot2", "Plot GSEAplot2", style = "margin-top:30px"))
           ),
...
server <- function(input, output, session){
  ...
  Fplot <- eventReactive(input$ftrplot, {
    req(seurat())
    seurat <- seurat()
    
    FeaturePlot(seurat, features = input$genename, cols = c("lightgray","darkred"), pt.size = 1)+
      my_umap_theme()
  })
  
  output$plotfeatureplot <- renderPlot({
    req(Fplot())
    Fplot()
  ...
  Dplot <- eventReactive(input$display.dplot, {
    ...
    DotPlot(seurat,
      ...
      labs(title = paste0("Genes expression within :  ", input$Sel.metadata2),
           x="",
           y="")+
      scale_color_gradientn(colours = c("orange","white","navy"),
                            values = c(0,0.5,1))+
  ...
  observeEvent(input$add.markers, {
    ...
    ranked_genes <- rankingdata$ranking %>% setNames(rankingdata$ENTREZID)
  ...
  Corr_plot <- eventReactive(input$go.corrplot, {
    ...
    colnames(term_sim_mat) <- ifelse(nchar(colnames(term_sim_mat)) <= 20, 
                                     colnames(term_sim_mat), 
                                     paste0(substr(colnames(term_sim_mat),1,18),"...."))
  ...
  observeEvent(List_markers(), {
    req(List_markers())
    List_markers <- List_markers()
    updateSelectInput(session, "gsea.sel", choices = names(List_markers), selected = "")
  })
  ...
  GSEAobj <- eventReactive(input$run.gsea, {
    ...
    gseGO(geneList  = List_markers[[input$gsea.sel]][["Ranked_genes"]],
          OrgDb        = org.Hs.eg.db,
          ont          = "BP",
  ...
  output$gseaTab <- renderDataTable({
    ...
    res <- GSEAobj@result
    if(nrow(res) > 0){
      res %>% dplyr::select(Description, setSize, NES, p.adjust, rank) %>% 
        datatable(options = list(pageLength = 5, scrollX = TRUE))
    }
  })
  ...
  #### p3: gseaplot        --------------------------------------------------------
  observeEvent(GSEAobj(), {
    req(GSEAobj())
    res <- GSEAobj()@result
    
    updateSelectInput(session, "terms.gsea.sel", choices = res[["Description"]])
  })
