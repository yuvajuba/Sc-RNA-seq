...
subtitle: "Library 3 pre-processing"
author: "Juba"
...
## Set chunk options
knitr::opts_chunk$set(message = FALSE, warning = FALSE, echo = FALSE, error = FALSE, fig.height = 6, fig.width = 7)
...
# Directories :
# =========== :
out_dir <- "d:/Projects/Thesis/Lib3/out_obj/"
fig_dir <- "d:/Projects/Thesis/Lib3/out_fig/"
seu_dir <- "d:/Projects/Thesis/Lib3/SeuratObject/"
```
...
SeuratObj_filt@meta.data <- SeuratObj_filt@meta.data %>% 
  mutate(
    MyClusters = case_when(
      Clust_res0.3 %in% c("5R","0R") ~ "R1",
      Clust_res0.3 %in% c("1R","2R") ~ "R2",
      Clust_res0.3 %in% c("5D","0D") ~ "D1",
      Clust_res0.3 %in% c("2D","4D") ~ "D2",
      Clust_res0.3 == "1D" & umap_1 > 0 ~ "D2",
      TRUE ~ "D3"
    )
  )
...
SeuratObj_filt@meta.data <- SeuratObj_filt@meta.data %>% 
  mutate(
    MyClusters = case_when(
      MyClusters %in% c("D1","D2") & umap_1 > -1.8 & umap_2 > -2 ~ "D",
      MyClusters %in% c("R1","R2") & umap_1 > 0 & umap_2 > -2 ~ "R_in_D",
      MyClusters %in% c("D1") & umap_1 < -1.8 ~ "D_in_R1",
      MyClusters %in% c("D2") & umap_2 < -2 ~ "D_in_R2",
      TRUE ~ MyClusters
    )
  )
...
SeuratObj_filt@meta.data <- SeuratObj_filt@meta.data %>% 
  mutate(MyClusters = case_when(
    MyClusters == "D_in_R1" ~ "D2",
    MyClusters == "R_in_D" ~ "R3",
    MyClusters %in% c("D","D_in_R2") ~ "D1",
    TRUE ~ MyClusters
  ))
...
unique_cl <- tmp$v_call[which(tmp$cl == "unique")]
low_cl <- tmp$v_call[which(tmp$cl == "few")]

SeuratObj_filt@meta.data <- SeuratObj_filt@meta.data %>% 
  mutate(TCR = case_when(is.na(v_call) ~ "Missing",
                         v_call %in% unique_cl ~ "Unique",
                         v_call %in% low_cl ~ "Low_count",
                         TRUE ~ v_call))
...
SeuratObj_filt@meta.data %>% 
  ...
  geom_text(aes(label = ifelse(n > 200, n, "")),
            ...
            size = 4,
            fontface = "bold")+
  geom_text_repel(aes(label = ifelse(n < 200 & n > 20, n,"")), 
...
SeuratObj_filt@meta.data %>% 
  ...
  geom_text(aes(label = ifelse(n > 100, n, "")),
            ...
            size = 4,
            fontface = "bold")+
  geom_text_repel(aes(label = ifelse(n <= 100 & n >= 20, n,"")), 
                  position = position_stack(vjust = 0.5), 
                  color = "black", size = 4, fontface = "bold", 
                  box.padding = 0.5,    
                  point.padding = 0.6,  
